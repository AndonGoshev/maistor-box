...
class PrivateViewsPermissionRequiredMixin:
    def dispatch(self, request, *args, **kwargs):

        def get_account():

            if request.user.user_type == 'contractor_user':
                account = get_object_or_404(ContractorUserModel, pk=kwargs['id']).user
        ...
        # First of all we are checking if the user is authenticated. If its not
        # it's for sure that they don't have permission to land on this private
        # part of the website!
        if not request.user.is_authenticated:
            raise PermissionDenied(f'Not authenticated user tried to access a private resource by '
                                   f'forced browser attack. Targeted page = {get_page_url()}')
